...
/**
 * Handles inconsistent API timing:
 * Some routes use minutes (e.g., 59.77 for 12km)
 * Others use seconds (e.g., 5991.05 for 29km)
 */
function formatDuration(raw) {
  if (!raw) return "N/A";
  // Heuristic: If it's a large number for a standard distance, it's likely seconds
...
// Derive difficulty from distance in meters
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d="M0,210 C80,200 120,220 200,215 C280,210 320,190 400,195 C480,200 520,185 600,180 C680,175 740,185 800,180"
          stroke="#e8e0d0"
          strokeWidth="8"
          fill="none"
          opacity="0.7"
        />
        <path
          d="M80,180 C130,165 180,155 240,160 C300,165 350,175 400,168 C450,161 500,150 560,155"
          stroke="#FF7F11"
          strokeWidth="4"
          fill="none"
          strokeLinecap="round"
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="font-bold text-brand-dark text-base leading-tight truncate mr-2">
          {route.name || "Untitled Route"}
        </span>
        <span
          className={`text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0 ${diffStyles[difficulty]}`}
        >
          {difficulty}
        </span>
      </div>

      <p className="text-xs text-brand-dark/45 mb-3 truncate">
      ...
      <div className="flex items-center gap-4 text-sm text-brand-dark/55">
        ...
        <span title="Distance">📏 {formatDistance(route.distance)}</span>
      </div>
    ...
    >
      ...
      >
        ...
        <p className="text-brand-dark/55 text-sm mb-6 leading-relaxed">
          Create a free account to explore full route details, track your rides,
          and join the community.
        </p>
        <div className="flex flex-col gap-3">
          <a
            href="/register"
            className="bg-brand-orange text-white font-bold py-3 px-6 rounded-xl hover:bg-brand-orange/90 transition-colors"
          >
            Get Started Free
          </a>
          <a
            href="/login"
            className="border border-brand-dark/20 text-brand-dark font-semibold py-3 px-6 rounded-xl hover:bg-brand-cream transition-colors"
          >
            Log In
          </a>
        </div>
        <button
          onClick={onClose}
          className="mt-5 text-xs text-brand-dark/35 hover:text-brand-dark/60 transition-colors"
        >
          Maybe later
        </button>
      </div>
...
export default function HomePage() {
  ...
  headers: {
    Authorization: `Bearer ${token}`,
      ...
      .then((res) => res.json())
      .then((data) => {
        // According to your API result: data = { message, count, routes: [...] }
        if (data && data.routes) {
          // Display the 6 most recent routes
          setRoutes(data.routes.slice(0, 6));
        }
        setLoading(false);
      })
      .catch((err) => {
  ...
  const features = [
    {
      ...
      desc: "Browse community-shared paths or plan your own with elevation data.",
    },
    {
      icon: "🚴",
      title: "Track Rides & CO₂ Saved",
      desc: "Log every ride automatically and watch your carbon footprint drop.",
    },
    {
      icon: "⚠️",
      title: "Report Road Hazards",
      desc: "Flag potholes or dangerous junctions to keep every cyclist safer.",
    },
    {
      icon: "🏆",
      title: "Challenges",
      desc: "Join weekly challenges and compete with cyclists across your city.",
    },
  ];
  ...
  const steps = [
    {
      ...
      desc: "Create your account and set your preferred cycling style.",
    },
    {
      n: "02",
      title: "Find a route",
      desc: "Browse public routes or design your own from scratch.",
    },
    {
      n: "03",
      title: "Ride & Track",
      desc: "Hit the road, earn points, and climb the leaderboard.",
    },
  ];
  ...
  return (
    <div className="font-sans bg-brand-cream min-h-screen overflow-x-hidden">
        ...
        <div className="relative max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-extrabold text-brand-dark leading-[1.08] tracking-tight mb-5">
              Ride Green.
              <br />
              <span className="text-brand-orange">Track Impact.</span>
              <br />
              Build Community.
            </h1>
            <p className="text-brand-dark/60 text-lg leading-relaxed mb-8 max-w-md">
              Discover cycling routes, measure the CO₂ you save, and report
              hazards in real time.
            </p>
            <div className="flex flex-wrap gap-3">
              <a
                href="/register"
                className="bg-brand-orange text-white font-extrabold px-8 py-4 rounded-xl hover:bg-brand-orange/90 shadow-lg shadow-brand-orange/25"
              >
                Get Started Free
              </a>
              <a
                href="/login"
                className="bg-white border border-brand-dark/20 text-brand-dark font-semibold px-8 py-4 rounded-xl hover:bg-brand-cream"
              >
                Log In
              </a>
            </div>
      ...
      <section className="bg-brand-dark py-16 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 divide-y sm:divide-y-0 sm:divide-x divide-brand-sage/20">
            <StatCounter
              value={stats.totalDistance || 142000}
              label="Total km cycled"
            />
            <StatCounter
              value={stats.co2Saved || 28000}
              label="kg of CO₂ saved"
              suffix=" kg"
            />
            <StatCounter
              value={stats.activeCyclists || 3200}
              label="Active cyclists"
            />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((f, i) => (
              <div
                key={i}
                className="bg-brand-cream rounded-2xl p-6 hover:bg-brand-sage/30 transition-colors"
              >
                <div className="text-4xl mb-4">{f.icon}</div>
                <h3 className="font-extrabold text-brand-dark text-base mb-2">
                  {f.title}
                </h3>
      ...
      {/* Public Routes from DB */}
      <section className="py-20 px-6 md:px-12 lg:px-24 bg-brand-cream">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-end justify-between mb-10 gap-4 flex-wrap">
            <div>
              <span className="inline-block bg-white text-brand-dark text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-full mb-3">
                Community Routes
              </span>
              <h2 className="text-4xl font-extrabold text-brand-dark tracking-tight">
                Recent public rides
          ...
          {loading ? (
            <div className="text-center py-20 text-brand-dark/40 font-medium">
              Loading routes from community...
            </div>
          ) : (
      ...
      <footer className="bg-brand-dark border-t border-white/10 py-10 px-6 md:px-12">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6 flex-wrap">
          ...
          <p className="text-brand-sage/35 text-xs">
            © {new Date().getFullYear()} CycleGreen. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
